# --------------------------------------------------------------
# test_pld_tree.py  –  Prompt Lookup Decoding (PLD) + Tree-based Verification 測試腳本
# --------------------------------------------------------------
# 此腳本會：
...
#   5️⃣ 執行 PLD + Tree‑based Verification 的 HuggingFace LLM（使用自訂的 tree‑based 解碼）
#   6️⃣ 比較兩種方式的延遲與輸出相似度
...
# ────────────────────────────────────────
# 5️⃣  PLD 基礎類別（與 test_pld.py 相同，提供給 Tree 版使用）
# ────────────────────────────────────────
...
# ────────────────────────────────────────
# 6️⃣  PLD + Tree‑based Verification 類別
# ────────────────────────────────────────
class HFPLDTreeLLM(HFPLDLLM):
    """
    使用 PLD 作為草擬模型，並以目標模型進行樹狀驗證（深度 2）。
    實作參考 SpecInfer/EAGLE 的想法：先用 PLD 產生候選樹，
    再用目標模型一次驗證多個路徑，接受最長匹配的前綴。
    本實作採用逐位置驗證，但會嘗試一次驗證兩層（若可能）。
    """

    def _get_pld_candidates(self, context_token_ids: List[int]) -> Set[int]:
        """
        根據 PLD 機制，回傳在給定 context 下可能的下一個 token 集合。
        方法：尋找結尾的 n-gram（長度 1..self.prompt_lookup_num_tokens）在
        前文（不含目前結尾）中的出現，取緊距該 n-gram 之後的 token 作為候選。
        """
        if not context_token_ids:
            return set()
        candidates = set()
        seq = torch.tensor(context_token_ids, device=self.device)
        seq_len = seq.size(0)
        max_lookback = min(self.prompt_lookup_num_tokens, seq_len - 1)
        for l in range(1, max_lookback + 1):
            # 取最後 l 個 token 作為查詢
            query = seq[-l:]
            # 在 [0 : seq_len - l) 範圍內尋找匹配
            # 這裡採用簡單的逐位比較（因為序列長度不會太長）
            start_positions = []
            for i in range(0, seq_len - l):
                if torch.equal(seq[i:i+l], query):
                    start_positions.append(i)
            for start in start_positions:
                nxt_pos = start + l
                if nxt_pos < seq_len:
                    cand_id = seq[nxt_pos].item()
                    candidates.add(cand_id)
        return candidates

    def _get_model_next_token_logits(self, input_ids: torch.Tensor) -> torch.Tensor:
        """
        給定輸入 token ids (shape: [1, seq_len])，返回下一個 token 的 logits (shape: [vocab_size])
        """
        with torch.no_grad():
    ...
    def _generate_tree(self, prompt: str) -> str:
        """
        使用樹狀驗證產生文字，最多 self.max_new_tokens 個 token。
        回傳生成的文字字串。
        """
        # 初始化
        input_ids = self.tokenizer(prompt, return_tensors="pt").input_ids.to(self.device)
        generated_ids: List[int] = []
        # 為了效能，我們先將 prompt 的 tokenize 結果快取為 tensor
        # 但在每次迭代中我們需要拼接已生成的 token，這樣開銷仍可接受（因為序列不會太長）

        while len(generated_ids) < self.max_new_tokens:
            # 當前完整序列 = prompt + 已生成的 token
            current_ids = torch.cat([
                input_ids,
                torch.tensor(generated_ids, device=self.device, dtype=torch.long).unsqueeze(0)
            ], dim=1) if generated_ids else input_ids

            # 取得第 1 層候選詞（基於當前完整序列）
            level1_candidates = self._get_pld_candidates(current_ids.squeeze(0).tolist())
            if not level1_candidates:
                # 若沒有候選詞，退回到模型的貪婪預測
                logits = self._get_model_next_token_logits(current_ids)
                next_token = torch.argmax(logits).unsqueeze(0)
                generated_ids.append(next_token.item())
                continue

            # 取得模型在當前位置的 logits（未套用 PLD）
            logits = self._get_model_next_token_logits(current_ids)
            probs = torch.softmax(logits, dim=-1)
            top1_token = torch.argmax(probs).item()
            # 取 top-5 作為備註（實際上我們只需要 top-1 來決策，但亦可考慮更寬鬆的條件）
            # top5 = torch.topk(probs, 5).indices.tolist()

            # 檢查第一層是否命中
            if top1_token in level1_candidates:
                # 接受第一個 token
                accepted_first = True
                # 準備第二層：先假設第一個 token 已被接受，然後再次取得該位置的候選詞
                # 建立新的 context = current_ids + [top1_token]
                candidate_first_id = top1_token
                second_context_ids = torch.cat([
                    current_ids,
                    torch.tensor([[candidate_first_id]], device=self.device, dtype=torch.long)
                ], dim=1)
                # 第二層候選詞：基於 第二個位置 的 context（即加入第一個 token 後）
                level2_candidates = self._get_pld_candidates(second_context_ids.squeeze(0).tolist())
                # 取得第二個位置的模型 logits
                logits2 = self._get_model_next_token_logits(second_context_ids)
                probs2 = torch.softmax(logits2, dim=-1)
                top2_token = torch.argmax(probs2).item()

                if top2_token in level2_candidates:
                    # 第二個也命中：接受兩個 token
                    generated_ids.append(candidate_first_id)
                    generated_ids.append(top2_token)
                    # 已經產生了 2 個 token，繼續外層 while 迴圈（可能再次迭代）
                    continue
                else:
                    # 第二個未命中：只接受第一個 token，第二個將在下一輪重新處理
                    generated_ids.append(candidate_first_id)
                    # 只有第一個 token 被接受，輪到下一個位置時會重新計算候選詞
                    continue
            else:
                # 第一個未命中：使用模型的預測（或可考慮從候選中選機率最高的，這裡直接用模型 top-1）
                generated_ids.append(top1_token)
                # 已經用模型的額外 token 前進一個位置，繼續
                continue
    ...
    def _call(self, prompt: str) -> str:
        """對外介面：使用樹狀驗證產生回覆"""
        return self._generate_tree(prompt)

# ────────────────────────────────────────
# 7️⃣  設定要測試的 HuggingFace 模型與 PLD 參數（樹版使用同樣參數）
# ────────────────────────────────────────
# 請依照您的硬體與需求自行修改以下兩行
...
for idx, q in enumerate(questions, start=1):
    ...
    # ---- PLD + Tree‑based Verification ----
    ans_pld_tree, lat_pld_tree = hf_pld_tree_llm._call(prompt), 0.0
    # 我們需要計算延遲，因此改為計時版本
    t0 = time.perf_counter()
    ans_pld_tree = hf_pld_tree_llm._call(prompt)
    t1 = time.perf_counter()
    lat_pld_tree = t1 - t0
    pld_tree_latencies.append(lat_pld_tree)

    # ---- 相似度 ----
    sim = similarity(ans_ollama, ans_pld_tree)
    similarities.append(sim)

    print(
        f"[{idx:02d}] 問題: {q[:30]}... | "
        f"Ollama: {lat_ollama:.2f}s | PLD-Tree: {lat_pld_tree:.2f}s | "
        f"相似度: {sim:.3f}"
    )

# ---- 統計 ----
avg_ollama_lat = sum(ollama_latencies) / len(ollama_latencies)
avg_pld_tree_lat = sum(pld_tree_latencies) / len(pld_tree_latencies)
speedup = avg_ollama_lat / avg_pld_tree_lat if avg_pld_tree_lat > 0 else float("inf")
avg_sim = sum(similarities) / len(similarities)

print("\n=== 測試結果摘要 ===")
print(f"平均 Ollama 延遲: {avg_ollama_lat:.3f} 秒")
print(f"平均 PLD-Tree 延遲: {avg_pld_tree_lat:.3f} 秒")
print(f"平均加速比 (Ollama / PLD-Tree): {speedup:.2f}×"
      f"{' (PLD-Tree 更快)' if speedup > 1 else ''}")
print(f"平均輸出相似度: {avg_sim:.3f} (1.0 表示完全相同)")

# 結語：若速度顯著提升且相似度接近 1.0，則 PLD+Tree 在您的資料上是有效的。